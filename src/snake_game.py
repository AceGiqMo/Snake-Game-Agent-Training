...
class SnakeGame:
    ...
    def reset(self, map):
        ...
        self.snake = copy.deepcopy(self.snake_start_cells)
        ...
        self._actions = {}
        self._food_history = {}
        self._food_history[self.frame] = {"food_x": self.food[0], "food_y": self.food[1]}

    def _generate_food(self):
        """Generate food in a random free cell (not on snake or obstacles)."""
        snake_arr = np.array(self.snake)

        occupied = copy.deepcopy(SnakeGame.map)
        occupied[snake_arr[:, 0], snake_arr[:, 1]] = True
        free_mask = np.where(~occupied)

        return self.rng.choice(np.array([free_mask[1], free_mask[0]]).T, axis=0).tolist()
    ...
    def _handle_food_eating(self):
        ...
        if head == self.food:
            ...
            if self.super_food:
                self._food_history[self.frame]["super_food_x"] = self.super_food[0]
                self._food_history[self.frame]["super_food_y"] = self.super_food[1]
        ...
        # Eat super-food
            ...
            self._food_history[self.frame] = {"food_x": self.food[0], "food_y": self.food[1],
                                              "super_food_x": None, "super_food_y": None}
        ...
        # Time over (to avoid endless loop)
        if self.frame == 1000:
            self.game_over = True
    ...
    def _update_super_food(self):
        ...
        if not self.super_food and self.frame % 50 == 0:
            ...
            if self.super_food:
                ...
                self._food_history[self.frame] = {"food_x": self.food[0], "food_y": self.food[1],
                                                  "super_food_x": self.super_food[0],
                                                  "super_food_y": self.super_food[0]}
